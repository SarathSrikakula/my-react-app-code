...
export default function App() {
  const policyCodeRedux = useAppSelector((state: RootState) => state.general.policyCode);
  const dispatch = useAppDispatch();

  const policyAssigenedData = useAppSelector((state: RootState) => state.states.policyAssignedStates);
  const data = policyAssigenedData;

  useEffect(() => {
    const fetchData = async () => {
      dispatch(fetchpolicyAssignedStates(policyCodeRedux));
    };
...
// please write vitest tests for the above code